...
MRB_API void mix_plugin_add_event_listener(mrb_state *mrb, mrb_value plugin, const char *event_name, mrb_func_t callback) {
  ...
  mrb_funcall_with_block(mrb, plugin, mrb_intern_lit(mrb, "add_event"), 1, &sym_event_name, mrb_obj_value(proc));
...
MRB_API void mix_plugin_add_event_filter(mrb_state *mrb, mrb_value plugin, const char *event_name, mrb_func_t callback) {
  ...
  mrb_value env[] = { plugin, sym_event_name };
  struct RProc *proc = mrb_proc_new_cfunc_with_env(mrb, callback, sizeof(env) / sizeof(mrb_value), env);
  mrb_funcall_with_block(mrb, plugin, mrb_intern_lit(mrb, "add_event_filter"), 1, &sym_event_name, mrb_obj_value(proc));
